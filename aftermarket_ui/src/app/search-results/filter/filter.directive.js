...
class FilterDirectiveController {
    ...
    categoryFilter(selectedCategory) {
        ...
        if (SearchBarService.productLine && SearchBarService.productLine.id == 0) {
            ...
            let paramObj = {'filters':"", 'filterObject':"",cat1:selectedCategory.id, "cat2": "", "cat3":"", "from":"", "size":""};
            $state.go("searchResults", paramObj);
        } else {
            if (selectedCategory.select) {
                SearchBarService.productClass = selectedCategory;
                SearchBarService.productCategory = 0;
                let paramObj = {'filters':"", 'filterObject':"","cat2": selectedCategory.id, "cat3":"", "from":"", "size":""};
                $state.go("searchResults", paramObj);
            } else {
                SearchBarService.productClass = 0;
                let paramObj = {'filters':"", 'filterObject':"", "cat1":$stateParams.cat1, "cat2": "", "cat3":"", "from":"", "size":""};
                $state.go("searchResults", paramObj);
            }
        }
    ...
    subCategoryFilter(category, selectedSubCategory) {
        ...
        if (selectedSubCategory.select) {
            ...
            let paramObj = {'filters':"", 'filterObject':"", "cat2": category.id, "cat3": selectedSubCategory.id, "from":"", "size":"" };
            $state.go("searchResults", paramObj);
        } else {
            SearchBarService.productCategory = 0;
            let paramObj = {'filters':"", 'filterObject':"", "cat1":$stateParams.cat1, "cat2": category.id, "cat3": "", "from":"", "size":"" };
            $state.go("searchResults", paramObj);
        }
