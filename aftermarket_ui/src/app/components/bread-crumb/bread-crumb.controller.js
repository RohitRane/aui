        ...
        if ($state.is('searchResults')) {
            ...
            if ($stateParams.sort) {
                ...
                /*switch($stateParams.sort){
                    case "0":vm.sortItem = SearchBarService.getParticularSAttr($stateParams.sort).displayName;break;
                    case "1":break;
                    case "2":break;
                    case "3":break;
                }*/
            }
            //vm.sortItem = $stateParams.sort ? "Part Number:" + SearchBarService.getParticularSAttr($stateParams.sort).Type[0].toUpperCase() + SearchBarService.getParticularSAttr($stateParams.sort).Type.slice(1).toLowerCase() : vm.sortAttributes[0].displayName;
    ...
    sortAction(sortObj) {
        ...
        if (SearchBarService.sort) {
            switch (sortObj.displayName) {
                case 'Popularity': srt = '1'; break;
                case 'Part Number : Asc': srt = '2'; break;
                case 'Part Number : Desc': srt = '3'; break;
            }
        }
